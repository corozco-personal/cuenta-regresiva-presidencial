...
export async function GET() {
  try {
    ...
    const [opinionRows, submissionRows] = await Promise.all([
      db.select({ stance: opinions.stance, status: opinions.status, country: opinions.country, createdAt: opinions.createdAt }).from(opinions),
    ...
    return Response.json({
      ...
      opinions: { total: opinionRows.length, stances, moderation },
