...
export default function NavigationCatalog({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return <div className="nav-more-panel">
    {navigationGroups.map((group, index) => <section className={`nav-group${expandedGroup === index ? " open" : ""}`} key={group.title}>
      <header><button className="nav-group-toggle" type="button" aria-expanded={expandedGroup === index} onClick={() => setExpandedGroup(current => current === index ? null : index)}><ChevronDown size={16} /><span><strong>{group.title}</strong><small>{group.description}</small></span></button></header>
