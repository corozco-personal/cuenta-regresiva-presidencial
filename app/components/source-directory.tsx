...
export default function SourceDirectory() {
  ...
  return <div><p className="page-intro">El directorio usa únicamente dominios editoriales propios y verificables. Incluye agencias nacionales, medios públicos y cabeceras influyentes de cada mercado; excluye agregadores, redes sociales y blogs personales. Estar incluido no implica respaldo editorial.</p><label className="search-box source-page-search"><Search size={18} /><span className="sr-only">Buscar fuente, país o región</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar fuente, país o región" /></label>{state === "loading" && <p className="empty-stream">Cargando el directorio…</p>}{state === "error" && <p className="empty-stream">El directorio no está disponible temporalmente.</p>}{state === "ready" && <><p className="source-page-count">{filtered.length} fuentes encontradas</p><div className="subpage-links">{filtered.map((source) => <article key={source.domain}><small>{source.country} · {source.region}</small><h2>{source.label}</h2><p>{source.kind}. {source.criterion}</p><a href={`https://${source.domain}`} target="_blank" rel="noreferrer">{source.domain}<ExternalLink size={14} /></a></article>)}</div></>}</div>;
}
