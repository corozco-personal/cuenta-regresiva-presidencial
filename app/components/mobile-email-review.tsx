...
export default function MobileEmailReview() {
  ...
  return <main className="mobile-review-page"><section className="mobile-review-card"><header><span className="brand-mark">07</span><div><small>CUENTA PÚBLICA</small><strong>Revisión segura</strong></div><ShieldCheck /></header>
    {done ? <div className="mobile-review-result"><CheckCircle2 /><h1>Decisión registrada</h1><p>{done}</p><Link href="/">Volver al portal</Link></div> : error ? <div className="mobile-review-result error"><XCircle /><h1>Enlace no disponible</h1><p>{error}</p><Link href="/cotejo-7d41e9c2">Abrir el centro de cotejo</Link></div> : !item ? <p className="mobile-review-loading">Cargando aporte…</p> : <>
      ...
      <div className="mobile-review-actions">{params.intent === "approve" ? <button disabled={busy} className="approve" onClick={() => void decide("approve")}><CheckCircle2 />{busy ? "Guardando…" : "Confirmar aprobación"}</button> : <button disabled={busy} className="reject" onClick={() => void decide("reject")}><XCircle />{busy ? "Guardando…" : "Confirmar rechazo"}</button>}<Link href="/cotejo-7d41e9c2">Revisar en el dashboard</Link></div>
    </>}
  </section></main>;
}
